...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CHI unit input in lnReqAmount, we generate the URI with CHI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CHI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
