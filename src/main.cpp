...
// Copyright (c) 2013 CHICOIN developers
...
int64 GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 770,000 blocks which will occur approximately every 7 years.
...
static const int64 nTargetSpacing =  60 * 60;  // change diff every 168 block
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    while (nTime > 0 && bnResult < bnLimit)
    {
           ...
           nTime -= nTargetTimespan*diffactor;
    }
    if (bnResult > bnLimit)
        bnResult = bnLimit;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Go back by what we want to be 7 days blocks
    ...
    if (nActualTimespan < nTargetTimespan/diffactor)
        nActualTimespan = nTargetTimespan/diffactor;
    if (nActualTimespan > nTargetTimespan*diffactor)
        nActualTimespan = nTargetTimespan*diffactor;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CHICOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CHICOINMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running CHICOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CHICOINMiner terminated\n");
        throw;
    }
}
